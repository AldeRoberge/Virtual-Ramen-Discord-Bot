...
namespace VRDiscord.Channels.Roles
{
    ...
    /// </summary>
    public class LanguageChannel : ChannelGenerator
    {
        public override bool DeleteAllMessages => true;

        public override Channel Channel => new()
        {
            Id = ChannelConstants.Languages,
            Name = "「🌎」language",
            Topic = "Select your language.",
            Channels = Channels.Languages
        };

        public static List<LanguageSelectableRole> SelectableLanguagesRoles;

        static LanguageChannel()
        {
            SelectableLanguagesRoles = ReflectionUtils<LanguageSelectableRole>.Load();
        }
        ...
        public override void PopulateMessages(MessageContainer messageContainer)
        {
            ...
            Console.WriteLine("Loading " + SelectableLanguagesRoles.Count + " selectable roles...");

            List<Color> colors = GradientGenerator.GetRainbowGradient(SelectableLanguagesRoles.Count);

            int i = 0;

            List<LanguageSelectableRole> languageRoles = new List<LanguageSelectableRole>();

            foreach (LanguageSelectableRole selectableRole in SelectableLanguagesRoles)
            {
            ...
            // Add the emojis
            foreach (SelectableRole selectableRole in SelectableLanguagesRoles)
            {
